...
def create_tiff_from_tin_rio(triangles, output_tiff_path, output_base_path, x_res=1, y_res=1):
	"""
	...
	grid_x, grid_y = np.meshgrid(
		np.linspace(min(x), max(x), int(max(x)-min(x))),  # Adjust grid resolution as needed
		np.linspace(min(y), max(y), int(max(y)-min(y)))
	)

	# Interpolate using linear interpolation
	grid_z = griddata((x, y), z, (grid_x, grid_y), method='linear')#, fill_value=-9999)
...
def genBase(file, output_path, out_type='asc'):
	...
	if elevation_array.min() < 0:
		...
		elevation_array = elevation_array + np.abs(elevation_array.min()) + 50
