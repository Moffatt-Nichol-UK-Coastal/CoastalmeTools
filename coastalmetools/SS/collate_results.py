...
def collate_results(path,t, mesh=False):
    itters = len(t)
    # t = sim_times(itters)
    t = pd.DatetimeIndex(t)
    ...
    for index, row in df.iterrows():
        if len(row.paths) != len(t):
            ...
            for elm in row.paths:
                ...
                # mnm = (row.variables + '_' + str(count) + ".ply")
                
                # if mesh:
                #     nc_to_mesh((n_path / tnm), (m_path / mnm))
                #     # f = netCDF4.Daataset((m_path / mnm))
                #     mesh = meshio.read((m_path / mnm))
                #     # list_dm.append(f)
